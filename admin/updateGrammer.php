...
<div class="container-lg">
    <div class="row py-2 bg-light border-bottom">
        <div class="col">
            <h5>
            Граматтика жоопторду ондоо
            </h5>
        </div>
        <div class="col d-flex justify-content-end">
            <a class="btn btn-primary" href="grammer.php">Артка</a>
        </div>
    </div>
        ...
        <form action="" class="form-control bg-light shadow" style="min-width:300px; max-width: 800px;" method="post">
            <div class="input-group mb-3">
                ...
                <div class="row border m-1">
                    <?php
                        $get = $conn -> query("SELECT * FROM type WHERE type = 3");
                        $option = '<option value="">Тема танданыз...</option>';
                        while($row = mysqli_fetch_array($get)){
                            $option = $option.'<option value="'.$row["id"].'">'.$row["kg"].'</option>';
                        }
                        $get = $conn -> query("SELECT * FROM test WHERE type=3 and  id_code=$id_code LIMIT 30 ");
                        $i = 0;
                        while ($row = mysqli_fetch_array($get)) { 
                            $i++;
                            $a = ($row["ans"] == "A"?"checked":"");
                            ...
                            $id_type = $row["id_type"];
                            echo '
                            <div class="ansBlock row d-flex align-items-center my-1" title="Грамматика тест №'.$i.'">
